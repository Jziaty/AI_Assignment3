...
public class Airplane : MonoBehaviour 
{
	...
	private Airplane[] otherFlocks;         //Unity Flocks in the group
	...
	void Start ()
	{
		...
		if (transform.parent)
		{
			tempFlocks = transform.parent.GetComponentsInChildren<Airplane>();
		}

		//Assign and store all the flock objects in this group
		objects = new Transform[tempFlocks.Length];
		otherFlocks = new Airplane[tempFlocks.Length];

		for(int i = 0;i<tempFlocks.Length;i++)
		{
			objects[i] = tempFlocks[i].transform;
			otherFlocks[i] = (Airplane)tempFlocks[i];
		}
		...
		//in order to have each Airplane behave individually but with generic Flock characteristics.
		...
		for(int i = 0;i<objects.Length;i++)
		{
			...
			if (transform != transformComponent)
			{
				...
				if (d < followRadius)
				{
					...
					//Separation
					f = d / followRadius;
					Airplane tempOtherFlock = otherFlocks[i];
	...
	static void OnGUI() {
		...
		if (Application.isEditor) {
			GUI.Label (new Rect (10,50,250,50), "Flocking with Airplanes"); 
